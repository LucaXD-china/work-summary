...
> End-to-end AI data applications: the query side offers safe, auditable read-only data access; the delivery side offers governed, traceable report publishing.

## 总览 Overview

AI 数据应用由两大支柱组成，共享同一条原则——**把 AI 时代「产出更快」的能力，用平台化治理补上「安全、可审计、可追溯」这后半程**：

1. **AI 查询体系（Query）**：面向 Codex/Cursor 等 MCP 客户端的只读数据查询体系——统一查询网关负责执行与审计，元数据与策略服务提供事实元数据与查询策略，语义层技能包提供领域引导。
2. **报告交付与数据可视化（Delivery）**：面向 AI 编码时代的内部数据报告交付平台——分析师负责页面与指标计算，平台负责身份、权限、数据契约与可追溯发布。

---

# 第一部分 · AI 查询体系 AI Query System

## 目的与边界 Purpose & Boundaries

这里的 "AI query" 指一组面向 Codex、Cursor 或自定义 MCP 客户端的**安全数据查询工具与使用约束**，不是一个独立实现的 Agent。工具链由三类能力组成：
...
```mermaid
flowchart LR
    Client["AI Client<br/>Codex / Cursor"] --> Sem["语义层技能包<br/>usage rules / domain notes"]
    ...
    Query --> Admission["SQL Admission<br/>read-only / budgets / Hive guardrails"]
    Admission --> AuthZ["AuthZ<br/>permission service"]
    AuthZ --> Exec["Execution<br/>StarRocks / Hive / ClickHouse"]

    Overview --> Metadata["Metadata access<br/>permission-scoped"]
    Meta["元数据与策略服务<br/>table facts / freshness"] -. "best-effort facts" .-> Overview
    Exec --> Data["StarRocks / Hive / ClickHouse"]
    Query --> Obs["structured logs / metrics<br/>query ids / usage accounting"]
```
...
# 第二部分 · 报告交付与数据可视化 Report Delivery & Data Visualization
